...
func GenerateBrandRoutes(mainRouter *chi.Mux, service services.Service) {
	mainRouter.Route("/brands", func(router chi.Router) {
		...
		router.Post("/", func(w http.ResponseWriter, r *http.Request) {
			category, err := utils.DecodeBody[services.Brand](r, w)
			if err != nil {
			...
			err = service.CreateBrand(category)
			if err != nil {
				http.Error(w, err.Error(), http.StatusBadRequest)
		...
		router.Patch("/{id}", func(w http.ResponseWriter, r *http.Request) {
			...
			err = service.EditBrand(id, category)
			if err != nil {
				http.Error(w, err.Error(), http.StatusBadRequest)
