...
func (s *Service) GenerateProducts() ([]Product, error) {
	...
	for rows.Next() {
		var e Entity
		if err := rows.Scan(
			&e.ID, &e.Name, &e.Description, &e.ImageID, &e.Price, &e.Priority,
			&e.ParentID, &e.Show, &e.EntitySlug,
	...
	for _, generatorV := range generatorEntities {
		for _, generatableV := range generatableProducts {
			...
			err = tx.QueryRow(ctx, insertOrUpdateQuery,
				...
				fmt.Sprintf("%s_%s", generatableV.Slug.String, generatorV.EntitySlug.String),
...
func (s *Service) ProductsInEntity(entity_id string) ([]Product, error) {
	...
	for rows.Next() {
		var product Product
		if err := rows.Scan(&product.ID, &product.Name, &product.Description, &product.Info, &product.Price, &product.Count, &product.CategoryID, &product.BrandID, &product.EntityID, &product.Slug, &product.CreatedAt, &product.UpdatedAt, &product.Generatable, &product.Generated, &product.ImageID, &product.ImageUrl, &product.Show, &product.ImageIDs, &product.Images); err != nil {
			return []Product{}, err
		}
